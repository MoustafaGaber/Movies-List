class MovieExpLoader {
  ...
  setupEventListeners() {
    ...
    //   .addEventListener("click", () => this.showWatchlist());
  }
  async loadGenres() {
    try {
  ...
  createTrendingCard(movie, rank) {
    ...
    const movieData = JSON.stringify(movie).replace(/"/g, "&quot;"); // تحويل الكائن لنص آمن
    const isAdded = this.watchlist.some((m) => m.id === movie.id);
    const heartIcon = isAdded ? "❤️" : "🤍";
    return `
        <div class="trending-card">
           <img src="${posterPath}" alt="${
      movie.title
    } Poster" class="movie-poster" loading="lazy" onerror="this.src='${
      this.FALLBACK_IMAGE
    }'"/>
          ...
          <div class="trending-overlay">
                ...
                onclick="window.movieApp.toggleWatchlist(${movieData}, this)">
                ${heartIcon}
                </button>
          </div>
  ...
  showWatchlist() {
      ...
      // عرض الأفلام المخزنة في المصفوفة
      this.displayMovies(this.watchlist, "moviesGrid");
  }
  // دالة إضافة/حذف من القائمة
  toggleWatchlist(movieId, btnElement) {
    // نحتاج لجلب بيانات الفيلم أولاً (بسيط للتبسيط)
    const index = this.watchlist.findIndex((m) => m.id === movieId);
    if (index === -1) {
      // في الواقع ستحتاج لتخزين بيانات الفيلم كاملة عند عرضه أول مرة
      // هنا سنفترض أننا سنخزن المعرف (ويمكنك تطويرها لتخزين الكائن كاملاً)
  ...
  toggleWatchlist(movieObj, btnElement) {
    ...
    if (index === -1) {
      // إضافة الفيلم كاملاً للقائمة
      this.watchlist.push(movieObj);
      btnElement.innerHTML = "❤️";
      btnElement.classList.add("active");
    } else {
      // إزالة الفيلم من القائمة
      this.watchlist.splice(index, 1);
      btnElement.innerHTML = "🤍";
      btnElement.classList.remove("active");

      // إذا كنا داخل صفحة "قائمتي" حالياً، نقوم بتحديث العرض فوراً بعد الحذف
      if (
        document
          .getElementById("randomSectionTitle")
          .textContent.includes("My Watchlist")
      ) {
        this.showWatchlist();
      }
    }

    // حفظ القائمة المحدثة في ذاكرة المتصفح
    localStorage.setItem("myWatchlist", JSON.stringify(this.watchlist));
  }
}
